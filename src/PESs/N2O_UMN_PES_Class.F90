...
#include "../qct.inc"
  ...
  ! Pre-processor conditional compilation statement to avoid full use of N2-O2 PES
#define N2O_ONLY 
  ...
  private
  public :: N2O_UMN_PES_Type

  !-------------------------!
  ! Definition of N2O Potential Energy Surface (PES) class. This is based on the PES for the N2O2 system 
  ! which adopts the following storage scheme for the six inter-atomic distances
  !  r1 = r(O1O2)    r2 = r(O1N3)
  !  r3 = r(O1N4)    r4 = r(O2N3)
  !  r5 = r(O2N4)    r6 = r(N3N4)
  type, extends(PES_Type) :: N2O_UMN_PES_Type
    real(rkp) ,dimension(256) :: C    !     
    real(rkp), dimension(6)   :: a    ! a  [A]
    real(rkp), dimension(6)   :: ab   ! ab [A^2]
    real(rkp), dimension(6)   :: re   ! re [A]
    integer                   :: iN2
    integer                   :: iNO
    integer                   :: jNO
    integer                   :: nC_pts
  contains
    procedure :: Initialize     => Initialize_N2O_UMN_PES
    procedure :: Output         => Output_N2O_UMN_PES
    procedure :: Compute        => Compute_N2O_UMN_PES_1d
    procedure :: Potential      => N2O_UMN_Potential_From_R
    procedure :: DiatPotential  => N2O_UMN_Potential_From_R_OnlyDiat
    procedure :: TriatPotential => N2O_UMN_Potential_From_R_OnlyTriat
  end type
  ...
  real(rkp), parameter :: Eref = -0.191619504727_rkp

  ! De(N2) + De(O2) = 228.4 + 120.143 kcal/mol 
  !real(rkp), parameter :: TotDiss = 348.643_rkp
  real(rkp) ,parameter :: VTotDiss = 0.0!348.643_rkp 

  ! Large distance used to pull the first O atom (O1) away in order 
  ! to use the N2O2 PES to describe the N2O system
  real(rkp), parameter :: large_dist = 1.d10
...
Subroutine Initialize_N2O_UMN_PES( This, Input, Atoms, iPES, i_Debug )
  ...
  character(:), allocatable                 :: UMN_N2O_file
  character(*), parameter                   :: Name_PES = 'N2O_UMN'
  character(80)                             :: line
  ...
  if (i_Debug_Loc) call Logger%Entering( "Initialize_N2O_UMN_PES" )
 ...
 ! ==============================================================================================================
  ...
  open( File=UMN_N2O_file, NewUnit=Unit, status='OLD', iostat=Status )
  if (Status/=0) call Error( "Error opening file: " // UMN_N2O_file )
  ...
  ! C
  read(Unit,*) This%nC_pts
  read(Unit,*) ( This%C(k), k = 1,This%nC_pts )
...
Subroutine Output_N2O_UMN_PES( This, Unit )

  class(N2O_UMN_PES_Type)                  ,intent(in)     ::    This
  integer                                 ,intent(in)     ::    Unit
  
  write(Unit,"('PES Name: ',g0)")  This%Name
  write(Unit,"('PES Model: ',g0)") This%Model
  write(Unit,"('N2O PES with cta bug fixed')")
  
End Subroutine
!--------------------------------------------------------------------------------------------------------------------------------!

!________________________________________________________________________________________________________________________________!
Function N2O_UMN_Potential_From_R( This, R, Q ) result( V )

  class(N2O_UMN_PES_Type)                       ,intent(in)  ::    This   !< PES class       
  ...
  integer                                             :: i  
  real(rkp)                                           :: VDiat, Vd_N2, Vd_NO1, Vd_NO2
  real(rkp), dimension(6)                             :: rMs
  real(rkp), dimension(0:17)                          :: rM
  real(rkp) ,dimension(6)                             :: Rang  
  real(rkp), dimension(1:256)                         :: B
  real(rkp), dimension(0:2253)                        :: P
  
  ! Perform conversion [Bohr] -> [A]
  ! Initialize pairs with large distances
  Rang = large_dist
  ...
  !  r3 = r(O1N4)    r4 = r(O2N3)
  !  r5 = r(O2N4)    r6 = r(N3N4)
  ! Set coordinates (atom O1 is pushed far away)
  Rang(4) = R(this%iNO)*B_To_Ang
  Rang(5) = R(this%jNO)*B_To_Ang
  Rang(6) = R(this%iN2)*B_To_Ang
  ...
  do i = 1,6
#endif  
     rMs(i) = exp(-(Rang(i) - This%re(i))/This%a(i) - (Rang(i) - This%re(i))**2.0d0/This%ab(i))
  enddo

  !-------------------------!
  ! Compute potential here
  ! Evaluate 2-body interactions
  ! - N3-N4
  ! - N3-O1
  ! - N4-O1
  ! Evaluate 2-body interactions
  VDiat =         This%Pairs(This%iNO)%Vd%DiatomicPotential(R(This%iNO))
  VDiat = VDiat + This%Pairs(This%jNO)%Vd%DiatomicPotential(R(This%jNO))
  VDiat = VDiat + This%Pairs(This%iN2)%Vd%DiatomicPotential(R(This%iN2))
  

  ! Evaluate basis functions
  if (This%Model == 'UMN_13A1') then
    call Unified_1(This, RAng, rMs, rM, P, B)
  else if (This%Model == 'UMN_13A2') then
    call Unified_2(This, RAng, rMs, rM, P, B)
  end if

  !-------------------------!
  ! Evaluate V by taking the product of C and Basis function array, then add 
...
!________________________________________________________________________________________________________________________________!
Function N2O_UMN_Potential_From_R_OnlyDiat( This, R, Q ) result( V )
  ...
  V = This%Pairs(This%iNO)%Vd%DiatomicPotential(R(This%iNO))
  V = V + This%Pairs(This%jNO)%Vd%DiatomicPotential(R(This%jNO))
  V = V + This%Pairs(This%iN2)%Vd%DiatomicPotential(R(This%iN2))
  
End Function
!--------------------------------------------------------------------------------------------------------------------------------!

!________________________________________________________________________________________________________________________________!
Function N2O_UMN_Potential_From_R_OnlyTriat( This, R, Q ) result( V )
  ...
  do i =1,6
#endif
     rMs(i) = exp(-(RAng(i) - This%re(i))/This%a(i) - ((RAng(i) - This%re(i))**Two)/This%ab(i))
  end do
  if (This%Model == 'UMN_13A1') then
    call Unified_1(This, RAng, rMs, rM, P, B)
  else if (This%Model == 'UMN_13A2') then
...
!________________________________________________________________________________________________________________________________!
Subroutine Compute_N2O_UMN_PES_1d( This, R, Q, V, dVdR, dVdQ )
  ...
  integer                                            :: i, j, iN2, iNO, jNO
  real(rkp)                                          :: VDiat
  real(rkp) ,dimension(6)                            :: Vd
  real(rkp) ,dimension(6)                            :: dVDiat
  real(rkp) ,dimension(6)                            :: RAng
  real(rkp) ,dimension(6)                            :: rMs
  real(rkp) ,dimension(6)                            :: dV0
  real(rkp) ,dimension(0:17)                         :: rM
  real(rkp) ,dimension(0:2253)                       :: P
  real(rkp) ,dimension(256)                          :: B
#ifdef N2O_ONLY
  real(rkp) ,dimension(4:6,6)                        :: dMsdR
  real(rkp) ,dimension(4:6,256)                      :: dBdR 
#else  
  real(rkp) ,dimension(6,6)                          :: dMsdR
  real(rkp) ,dimension(6,256)                        :: dBdR   
#endif  
  ...
  ! Set coordinates (atom O1 is pushed far away)
  RAng    = large_dist
  RAng(4) = R(iNO)*B_To_Ang
  RAng(5) = R(jNO)*B_To_Ang
  RAng(6) = R(iN2)*B_To_Ang
  ...
  Vd     = Zero
  dVDiat = Zero
  call This%Pairs(iNO)%Vd%Compute_Vd_dVd( R(iNO), Vd(4), dVDiat(4) )
  ...
  do i = 1,6
#endif  
     dMsdR(i,i) = (-Two*(RAng(i)-This%re(i))/This%ab(i) - One/This%a(i) )*exp(-(RAng(i) - This%re(i))/This%a(i) - ((RAng(i) - This%re(i))**Two)/This%ab(i) )
  end do 

  if (This%Model == 'UMN_13A1') then
    call UnifiedBis_1(This, RAng, rM, P, dMsdR, dBdR)
  else if (This%Model == 'UMN_13A2') then
    call UnifiedBis_2(This, RAng, rM, P, dMsdR, dBdR)
  end if
  ...
  do j = 1,This%nc_Pts
     ...
     do i = 1,6
#endif     
        dV0(i) = dV0(i) + This%C(j)*dBdR(i,j)
     end do

  end do
  dV0 = dV0*KcmAng_To_HartB + dVDiat
...
! **************************************************************************************************************      
!________________________________________________________________________________________________________________________________!
Pure Subroutine Unified_1(This, R, rMs, rM, P, B)
...
!
  ...
  class(N2O_UMN_PES_Type), intent(in)       :: This  !< PES class
  real(rkp) ,dimension(6), intent(in)       :: R     !< inter-atomic distances [Angstrom]     
  real(rkp) ,dimension(6), intent(in)       :: rMs   !< exponential gaussian term
  real(rkp) ,dimension(0:17), intent(out)   :: rM
  real(rkp) ,dimension(0:2253), intent(out) :: P    
  real(rkp) ,dimension(256), intent(out)    :: B     !< basis functions   

  integer                                   :: i
  real(rkp) ,dimension(1:2254)              :: b1
  real(rkp) ,dimension(0:2153)              :: b2
...
Pure Subroutine Unified_2(This, R, rMs, rM, P, B)
! mixed exponential gaussian term ms = exp(-(r-ra)/a-(r-rb)**2/ab)
! ra:   reference bond length
! rb:   reference bond length
! a:    nonlinear paramter, unit Anstrom
...
#ifdef N2O_ONLY
  p    = Zero    ! Initialize all entries to zero
  p(0) = rm(0)
  p(1) = rm(1)
  p(2) = rm(2) + rm(3) + rm(4) + rm(5)
  p(4) = p(1)*p(2)
...
!________________________________________________________________________________________________________________________________! 
Pure Subroutine UnifiedBis_1( This, R, rM, P, dMsdR, dBdR )
...
!
  ...
  class(N2O_UMN_PES_Type)                   ,intent(in)     ::    This
  real(rkp) ,dimension(6)                   ,intent(in)     ::    R         ! Distances of atom-atom pairs [Angstrom]
  real(rkp) ,dimension(0:17)                ,intent(in)     ::    rM
  real(rkp) ,dimension(0:2253)              ,intent(in)     ::    P
#ifdef N2O_ONLY 
  real(rkp) ,dimension(4:6,6)               ,intent(in)     ::    dMsdR
  real(rkp) ,dimension(4:6,1:256)           ,intent(out)    ::    dBdR 
#else
  real(rkp) ,dimension(6,6)                 ,intent(in)     ::    dMsdR
  real(rkp) ,dimension(6,1:256)             ,intent(out)    ::    dBdR           
#endif

  integer                                                   ::    i ,j
#ifdef N2O_ONLY 
  real(rkp) ,dimension(4:6,1:2254)                          ::    db1dr
  real(rkp) ,dimension(4:6,1:2153)                          ::    db2dr
  real(rkp) ,dimension(4:6,0:17)                            ::    dMdR 
  real(rkp) ,dimension(4:6,0:2253)                          ::    dPdR
#else
  real(rkp) ,dimension(6,1:2254)                            ::    db1dr
  real(rkp) ,dimension(6,1:2153)                            ::    db2dr
  real(rkp) ,dimension(6,0:17)                              ::    dMdR 
  real(rkp) ,dimension(6,0:2253)                            ::    dPdR
#endif
...
#ifdef N2O_ONLY 
  dpdr = Zero ! Initialize all entries to zero

  i = 4
  dpdr(i,2) = dmdr(i,2) + dmdr(i,3) + dmdr(i,4) + dmdr(i,5)
  dpdr(i,4) = dpdr(i,1)*p(2) + p(1)*dpdr(i,2)
  dpdr(i,7) = dmdr(i,11) + dmdr(i,12)
  dpdr(i,11) = dpdr(i,2)*p(2) + p(2)*dpdr(i,2) - dpdr(i,7) - dpdr(i,6) - dpdr(i,5) - dpdr(i,7) - dpdr(i,6) - dpdr(i,5)
  ...
  dpdr(i,31) = dpdr(i,2)*p(11) + p(2)*dpdr(i,11) - dpdr(i,25) - dpdr(i,24) - dpdr(i,23)
  ...
  dpdr(i,61) = dpdr(i,7)*p(7) + p(7)*dpdr(i,7) - dpdr(i,34) - dpdr(i,34)
  dpdr(i,62) = dpdr(i,1)*p(31) + p(1)*dpdr(i,31)
  dpdr(i,65) = dpdr(i,7)*p(11) + p(7)*dpdr(i,11) - dpdr(i,44)
  dpdr(i,73) = dpdr(i,2)*p(31) + p(2)*dpdr(i,31) - dpdr(i,65) - dpdr(i,64) - dpdr(i,63)
  ...
  dpdr(i,151) = dpdr(i,2)*p(73) + p(2)*dpdr(i,73) - dpdr(i,141) - dpdr(i,140) - dpdr(i,139)
  ...
  dpdr(i,289) = dpdr(i,2)*p(151) + p(2)*dpdr(i,151) - dpdr(i,277) - dpdr(i,276) - dpdr(i,275)
  ...
  dpdr(i,517) = dpdr(i,2)*p(289) + p(2)*dpdr(i,289) - dpdr(i,503) - dpdr(i,502) - dpdr(i,501)
  ...
  dpdr(i,880) = dpdr(i,2)*p(517) + p(2)*dpdr(i,517) - dpdr(i,864) - dpdr(i,863) - dpdr(i,862)
  ...
  dpdr(i,1433) = dpdr(i,2)*p(880) + p(2)*dpdr(i,880) - dpdr(i,1415) - dpdr(i,1414) - dpdr(i,1413)
  ...
  dpdr(i,2252) = dpdr(i,2)*p(1433) + p(2)*dpdr(i,1433) - dpdr(i,2232) - dpdr(i,2231) - dpdr(i,2230) 
...
!________________________________________________________________________________________________________________________________! 
Pure Subroutine UnifiedBis_2( This, R, rM, P, dMsdR, dBdR )
! Subroutine to evalute the derivatives of MEG term X
! w.r.t. interatomic distance R(3)
! dmsdR:        Local variables, dirm(3,3)
! a:            Nonlinear pamameter(Angstrom)
