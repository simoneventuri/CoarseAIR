...
!!#define N4_EXP_FIX
  ...
  private
  public    ::    N3Half_NASA_PES_Type


  Type, extends(PES_Type) :: N3Half_NASA_PES_Type
    real(rkp) ::  shiftof0
  contains
    procedure ::  Initialize     =>    Initialize_N3Half_NASA_PES
    procedure ::  Output         =>    Output_N3Half_NASA_PES
    procedure ::  Compute        =>    Compute_N3Half_NASA_PES_1d
    procedure ::  Potential      =>    N3Half_NASA_Potential_From_R
    procedure ::  TriatPotential =>    N3Half_NASA_Potential_From_R_OnlyTriat
  End Type
...
! **************************************************************************************************************
! **************************************************************************************************************
Subroutine Initialize_N3Half_NASA_PES( This, Input, Atoms, iPES, i_Debug )

  use Input_Class                        ,only:  Input_Type
  use Atom_Class                         ,only:  Atom_Type
  use N2_LeRoy_DiatomicPotential_Class   ,only:  N2_LeRoy_DiatomicPotential_Type
  
  class(N3Half_NASA_PES_Type)               ,intent(inout)  ::    This
  ...
  integer                                                   ::    iP
  character(*)                    ,parameter                ::    Name_PES = 'N3Half_NASA'
  real(rkp)                                                 ::    Temp
  real(rkp)                                                 ::    veccsd
  real(rkp)                                                 ::    veuse
  ...
  if (i_Debug_Loc) call Logger%Entering( "Initialize_N3Half_NASA_PES" )
...
!#ifdef N4_PES_VERBOSE  
  write(6,*) ' * Initialize_N3Half_NASA_PES * ccsd potential at "re": ', veccsd 
!#endif

  call This%Pairs(1)%Vd%Compute_Vd_dVd( re, veuse, Temp )                                   
!#ifdef N4_PES_VERBOSE
  write(6,*) ' * Initialize_N3Half_NASA_PES * diatomic potential at "re": ',veuse  
!#endif

  This%shiftof0 = Two * (veccsd - veuse)                                            
!#ifdef N4_PES_VERBOSE
  write(6,*) ' * Initialize_N3Half_NASA_PES * shift of diatomic zero of energy: ', This%shiftof0/Two, '; shift of VTd: ', This%shiftof0
!#endif
...
Subroutine Output_N3Half_NASA_PES( This, Unit )

  class(N3Half_NASA_PES_Type)                 ,intent(in)     ::    This
...
Function N3Half_NASA_Potential_From_R( This, R, Q ) result( V )
  ...
  real(rkp) ,dimension(12)                                   ::    QTemp
  real(rkp)                                                  ::    t1, t2

!#ifdef N4_PES_VERBOSE  
  write(*,*) 'Inside N3Half_NASA_Potential_From_R'
  write(*,*) 'Q = ', Q
!#endif
  ...
  call n4fitd(This%shiftof0, QTemp(1:3), QTemp(4:6), QTemp(7:9), QTemp(10:12), V, dVdQin)
...
!________________________________________________________________________________________________________________________________!
Function N3Half_NASA_Potential_From_R_OnlyTriat( This, R, Q ) result( V )
...
!________________________________________________________________________________________________________________________________!
Subroutine Compute_N3Half_NASA_PES_1d( This, R, Q, V, dVdR, dVdQ )
  ...
  real(rkp) ,dimension(12)                                   ::    QTemp
  real(rkp) ,dimension(12)                                   ::    dVdQTemp

  dVdR = Zero
!#ifdef N4_PES_VERBOSE
  write(*,*) 'Inside Compute_N3Half_NASA_PES_1d'
!#endif
  ...
  atom_loop : do at = 1,3 
     dir_loop : do dir = 1,3
        dVdQTemp(3*(at - 1) + dir) = dVdQin(3*(at - 1) + dir) - dVdR4(dir)     
     enddo dir_loop
  enddo atom_loop
