...
Module RandomVectorBis_Module
  ...
  private
  public    ::    RandsetVecBis
  public    ::    RanddwsVecBis
  public    ::    RandNormBis
  
  integer   ,dimension(:,:) ,allocatable ::    jSeedStoreVec ! ~ iseed3
  real(rkp) ,dimension(:,:) ,allocatable ::    yMat          ! ~ x
...
Subroutine RandsetVecBis( iseed, i_Debug )
...
!calculations in base (ib=256=2**8)
  ...
  allocate( jSeedStoreVec(VecSize,8), stat=Status )
  if (Status/=0) call Error( "Error allocating jSeedStoreVec" )
  if (i_Debug_Loc) call Logger%Write( "Allocated jSeedStoreVec with dimensions = (", VecSize, ",8)")

  allocate( yMat(VecSize,100), stat=Status )
  if (Status/=0) call Error( "Error allocating yMat" )
  if (i_Debug_Loc) call Logger%Write( "Allocated yMat with dimensions = (", VecSize, ",100)")
  ...
  do iLine = 1,size(iSeed)
    ...
    do i = 1,8
      jSeedStoreVec(iLine,i) =   mod(is,ib)
      is                     =   is / ib
    end do
    if (i_Debug_Loc) write(Logger%Unit,"(6x,'[RandsetVec]: iseed3 = 2 * iseed + 1   in base 256 (i.e. base 2**8) = ',8i4)") (jSeedStoreVec(iLine,9-i), i=1,8)

    ! exercise random number generator in case iSeed not choosen between 0 and (2**31 - 1). put 100 random numbers into matrix yMat: this is new sequence
    do i = 1,size(yMat,2)
      yMat(iLine,i) = Rand12(jSeedStoreVec(iLine,:))
    end do
...
Function RanddwsVecBis(iLine) result(RandNum)
! function rand(idum)
...
!   return
  ...
  i               =   int( 99d0 * yMat(iLine,100) ) + 1
  RandNum         =   yMat(iLine,100)
  yMat(iLine,100) =   yMat(iLine,i)
  yMat(iLine,i)   =   Rand12(jSeedStoreVec(iLine,:))
...
Function RandNormBis(mean, sd) result( RandN )
