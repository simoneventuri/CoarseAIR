...
Module NO_Basel_DiatomicPotential_Class
...
! The LeRoy model is used.
  ...
  private
  public  ::    NO_Basel_DiatomicPotential_Type

  Type  ,extends(DiatomicPotential_Type)  ::    NO_Basel_DiatomicPotential_Type
  ...
  contains
    procedure          ::    Initialize        =>    Initialize_NO_Basel_DiatomicPotential
    procedure          ::    Compute_Vd_dVd    =>    Compute_Vd_dVd_NO_Basel
    procedure          ::    DiatomicPotential =>    DiatomicPotential_NO_Basel
    procedure ,private ::    ReadData_NO_Basel
  End Type
...
Subroutine Initialize_NO_Basel_DiatomicPotential( This, Input, SpeciesName, iMol, Mass1, Mass2, i_Debug )

  use Input_Class               ,only:  Input_Type

  class(NO_Basel_DiatomicPotential_Type),intent(out)  ::    This
  ...
  if (i_Debug_Loc) call Logger%Entering( "Initialize_NO_Basel_DiatomicPotential" )
  ...
  !   READING VARIABLES
  ! ==============================================================================================================
  call This%ReadData_NO_Basel( Input, i_Debug=i_Debug_Loc )
  ! ==============================================================================================================
...
!________________________________________________________________________________________________________________________________!
Subroutine ReadData_NO_Basel( This, Input, i_Debug )

  use Input_Class                         ,only:  Input_Type

  class(NO_Basel_DiatomicPotential_Type)  ,intent(inout)  ::    This
  type(Input_Type)                        ,intent(in)     ::    Input
  logical                       ,optional ,intent(in)     ::    i_Debug

  character(150)                                          ::    NO_Basel_Fldr
  integer                                                 ::    ii
  integer                                                 ::    Unit
  integer                                                 ::    Status
  logical                                                 ::    i_Debug_Loc
  ...
  NO_Basel_Fldr = trim(adjustl(Input%DtbPath))  // '/Molecules/NO/Basel/Params/'
  if (i_Debug_Loc) call Logger%Write( "Reading from Folder: ", trim(NO_Basel_Fldr) )

  if (.not. This%ker1) then
    inquire(file=trim(NO_Basel_Fldr)//"pes1.kernel", exist=This%ker1)   ! file_exists will be true if the file exists and false otherwise
  end if

  if (.not. This%ker2) then
    inquire(file=trim(NO_Basel_Fldr)//"pes2.kernel", exist=This%ker2)   ! file_exists will be true if the file exists and false otherwise
  end if

  !if (.not. ker3) then
  !  inquire(file=trim(NO_Basel_Fldr)//"pes23.kernel", exist=ker3)   ! file_exists will be true if the file exists and false otherwise
  !end if
  ...
  if (.not. This%stored ) then
    ...
    if (i_Debug_Loc) call Logger%Write( "Reading from File: ", trim(NO_Basel_Fldr)//"asymp.dat" )
    open(NewUnit=Unit, file=trim(NO_Basel_Fldr)//"asymp.dat", status='OLD', iostat=Status )
    if (Status/=0) call Error( "Error opening file: " // trim(NO_Basel_Fldr)//"asymp.dat" )
  ...
  !if (This%ker1 .and. This%ker2 .and. This%ker3 ) then
    if (This%ker1 .and. This%ker2 ) then
      ...
      call This%pes1%load_from_file(trim(NO_Basel_Fldr)//"pes1.kernel")
      call This%pes2%load_from_file(trim(NO_Basel_Fldr)//"pes2.kernel")
    !  call This%pes3%load_from_file(trim(NO_Basel_Fldr)//"pes23.kernel")
      This%kread = .true.
    else
      if (i_Debug_Loc) call Logger%Write( "Computing pes*.kernel Files" )

      call This%pes1%read_grid(trim(NO_Basel_Fldr)//"pes1.csv")
    !  print*,"IAMHERE"
    ...
    !
      call This%pes1%save_to_file(trim(NO_Basel_Fldr)//"pes1.kernel")
    

      call This%pes2%read_grid(trim(NO_Basel_Fldr)//"pes2.csv")
    !  print*,"IAMHERE"
    ...
    !
      call This%pes2%save_to_file(trim(NO_Basel_Fldr)//"pes2.kernel")


    !  call This%pes3%read_grid(trim(NO_Basel_Fldr)//"pes3.csv")
    !  print*,"IAMHERE"
    ...
    !
    !  call This%pes3%save_to_file(trim(NO_Basel_Fldr)//"pes3.kernel")
...
!________________________________________________________________________________________________________________________________!
Elemental Function DiatomicPotential_NO_Basel( This, R ) result( V )

  class(NO_Basel_DiatomicPotential_Type),intent(in)  :: This
  ...
  call diatno(R, This%nda2, This%darray2, VDiat, dVDiat)
...
!________________________________________________________________________________________________________________________________!
Elemental Subroutine Compute_Vd_dVd_NO_Basel( This, R, V, dV )
...
!________________________________________________________________________________________________________________________________!
Pure Subroutine diatno(r, nda2, darray2, ener, der)
  ...
  real(rkp)                 ,intent(in)  :: r
  integer                   ,intent(in)  :: nda2
  real(rkp) ,dimension(:,:) ,intent(in)  :: darray2
