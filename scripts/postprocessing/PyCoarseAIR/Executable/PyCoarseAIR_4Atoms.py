...
from Reading           import Read_Levels, Read_qnsEnBin, Read_Rates, Read_Rates_OldVersion#, Read_PartFuncsAndEnergies
...
##==============================================================================================================
InputData                       = inputdata()
InputData.OldVersionFlg         = False
InputData.SystNameLong          = 'NaNbNcNd_NASA'

InputData.TranVec               = np.array([20000.0])
InputData.T0                    = 300.0
NTran                           = np.size(InputData.TranVec)
InputData.iTVec                 = np.arange(NTran) + 1

InputData.QCTOutFldr            = WORKSPACE_PATH + '/CoarseAIR/N4_VS/Test/'
...
InputData.Kin.WriteDissInel_Flg = True 
...
## Grouping Rates
InputData.Kin.CGQCTFlg              = True
InputData.Kin.Groups.Flg            = [True, True, True, True, True, True]
InputData.Kin.Groups.Types          = ['VibSpecific', 'VibSpecific', 'VibSpecific', 'VibSpecific', 'VibSpecific', 'VibSpecific']  # '' / 'VibSpecific' / 'Adaptive'
InputData.Kin.Groups.PathsToMapping = ['', '', '', '', '', '']
InputData.Kin.Groups.FldrName       = '_VS'
#InputData.Kin.Groups.T0             = 300
InputData.Kin.Groups.Write_Flg      = True
InputData.Kin.Groups.WriteDiss_Flg  = True
InputData.Kin.Groups.WriteInel_Flg  = True
InputData.Kin.Groups.WriteExch_Flg  = True
...
print("\n[PyCoarseAIR]: Uploading Molecules Data")

Syst = Read_Levels(Syst, InputData)


for iMol in range(Syst.NMolecules): 
    print("[PyCoarseAIR]: iMol = " + str(iMol) ) 
    if (not (InputData.Kin.Groups.Types[iMol] == 'StS') ):
        Syst.Molecule[iMol].Grouped.Get_Mapping(Syst.Molecule[iMol].Levelvqn)


Syst = Read_qnsEnBin(Syst, Temp, InputData)


for iMol in range(Syst.NMolecules): 
    print("[PyCoarseAIR]: iMol = " + str(iMol) ) 
    if (InputData.Kin.Groups.Types[iMol] == 'StS'):
        if (InputData.Kin.Write_Flg):
            Write_PartFuncsAndEnergies(Syst, Temp, InputData, iMol)
    else:
        Syst.Molecule[iMol].Grouped.Compute_GroupProps(Syst.Molecule[iMol].NBins, Syst.Molecule[iMol].Levelg, Syst.Molecule[iMol].LevelEeV)
        Syst.Molecule[iMol].Grouped.Write_Groups_PartFuncsAndEnergies(Syst, InputData, Temp, iMol)
...
if (InputData.OldVersionFlg):
    Syst = Read_Rates_OldVersion(Syst, Temp, InputData)
else:
    Syst = Read_Rates(Syst, Temp, InputData)
