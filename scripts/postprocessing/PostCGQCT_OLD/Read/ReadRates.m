...
function [RatesMatrix, DissRates, ProcessesRates, KRec, KRecOverg] = ReadRates(iT, RatesMatrix, DissRates, ProcessesRates, iBinsStart, iBinsEnd, DissCorrectionFactor, NLevels, Levelg, LevelEeV)
  ...
  if ReadAllRatesFlg == -1
    ...
    KEq(1:NBins(iBinnedMol),1)       = ( EqVec(1:NBins(iBinnedMol)) ).^(-1);
    KRec(1:NBins(iBinnedMol),1)      = DissRates(1:NBins(iBinnedMol),1) ./ KEq(1:NBins(iBinnedMol),1);
    KRecOverg(1:NBins(iBinnedMol),1) = KRec(1:NBins(iBinnedMol),1)      ./ Levelg(1:NBins(iBinnedMol),1);
