...
global SigmaOn MergePairsFlg PlotPairUser iTraj iNode iProc tMin tMax vqnColor Angles MinPES MaxPES iLevelToPerturb EnergyUM EeVSpace iLevelsVec ...
       ...
       OutputFileName ForceFlg ParaViewFlg ConsiderDegFlg WriteExchangeFlg WriteOrigInvFlg WritePajekFlg PlotPColorFlg PlotSamplesFlg PlotMomentsFlg ...
       PlotPostCut BinPostCutVec iPESEnd iPESStart 
...
WriteRatesToNetworkFlg     = 0
  ...
  WriteOrigInvFlg  = [1,1]  % = [1,*] for writing Kij; = [*,1] for writing 1/Kij
  WritePajekFlg    = 1      % = 0 for writing a .csv list of edges; = 1 for writing a Pajek .net file 
LevToBinFlg                = 0
PlotStochProcessesRatesFlg = 1
  NPESTot        = 50 
  iPESStart      = 1
  iPESEnd        = 50
  PlotPColorFlg  = 0
  ...
  PlotMomentsFlg = 0
  PlotPostCut    = 1 
  BinPostCutVec  = [50, 150]
ComputePESToRatesCorrFlg   = 1
  TopPercentage_Rates   = [  1,  1];
  NbSampledPoints_Rates = [300,300];
  DistThreshold_Rates   = [0.15,0.15];
  ReadPESFlg        = true
  NHL               = [6,20,10,1];
  PreLogShift       = -3.5
...
for iT = 1:length(T0_Vec)
  ...
  if ReUpload > 0
    ...
    if PlotStochProcessesRatesFlg == 1
      ...
      if ReUpload >= 1
        ...
        ProcessesRates      = zeros(NBins(1),4,NTint); 17
  ...
  if WriteRatesToNetworkFlg == 1
    WriteRatesToNetwork(iT, NLevels, LevelEeV, Levelvqn, Leveljqn, rIn, Levelg, RatesMatrix, ProcessesRates, LevelEeVVib0, LevelEeVRot)
  end
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if PlotStochProcessesRatesFlg == 1
  ...
  iFigure = PlotStochProcessesRates(iFigure, NLevels, ProcessesRatesStoch, ProcessesRatesStochMmnts, Levelg, LevelEeV, EeV, DeltaEintDiss, Levelvqn, Leveljqn, LevToBin, rOut, rIn, Egam, Tau, LevelQ, dVIn, ddVIn, dVOut, ddVOut, dVdJIn, dVdJOut, QBins)   
end

if ComputePESToRatesCorrFlg == 1
  [iFigure] = ComputePESToRatesCorr(iFigure, RatesMatrixStoch, ProcessesRatesStoch, RGrid , EGrid, EGridMean, EGridSD, IniStateVec, FinStateVec, ProcVec, QBins, TopPercentage_Rates, NbSampledPoints_Rates, DistThreshold_Rates);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%