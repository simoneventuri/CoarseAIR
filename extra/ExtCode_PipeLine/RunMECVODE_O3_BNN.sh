...
export FldrName='_PES9_Varga'
export Tran_vec=(2500) 
export T0=1400
export PathToMECVODEFldr=$WORKSPACE_PATH/neqplasma_QCT/ME_CVODE
export PathToDtbFldr=$WORKSPACE_PATH/BNNPaper_Database/Run_0D/database/
export PathToRunFldr=$WORKSPACE_PATH/BNNPaper_Database/Run_0D/
...
function Call_MeCvode() {
  ...
  if [ $DissFlg -eq 0 ]; then
    export ExFldr=${PathToMECVODEFldr}/${System}/'BNNPaper_T'${TTran}'K_NoDiss'
  elif [ $InelFlg -eq 0 ] && [ $ExchFlg1 -eq 0 ] && [ $ExchFlg2 -eq 0 ]; then
    export ExFldr=${PathToMECVODEFldr}/${System}/'BNNPaper_T'${TTran}'K_OnlyDiss'
  else
    export ExFldr=${PathToMECVODEFldr}/${System}/'BNNPaper_T'${TTran}'K'
  fi  
