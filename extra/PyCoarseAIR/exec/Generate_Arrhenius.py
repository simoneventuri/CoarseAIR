...
from Computing         import Compute_Rates_Thermal_FromOverall, Compute_QSS, Compute_ArrheniusFit_Diss, Compute_ArrheniusFit_Inel
...
InputData.TranVec               = np.array([1500, 2500, 5000, 6000, 8000, 10000, 12000, 14000, 15000, 20000])
...
InputData.Kin.ReadArr_Flg       = True
InputData.Kin.WriteArr_Flg      = True
InputData.Kin.SaveArr_Flg       = True
...
if (InputData.Kin.ReadArr_Flg) or (InputData.Kin.SaveArr_Flg) or (InputData.Kin.WriteArr_Flg):
	
	if (InputData.Kin.Diss_Flg):
		Syst = Compute_ArrheniusFit_Diss(Syst, Temp, InputData)

	if (InputData.Kin.Inel_Flg) or (InputData.Kin.Exch_Flg):
		Syst = Compute_ArrheniusFit_Inel(Syst, Temp, InputData)
