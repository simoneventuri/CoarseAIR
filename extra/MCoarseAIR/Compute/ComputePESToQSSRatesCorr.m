function [iFigure] = ComputePESToQSSRatesCorr(iFigure, KDiss, KDissQSS, KExch, KExchQSS, RGrid, EGrid, EGridMean, EGridSD, TopPercentage, NbSampledPoints, DistThreshold)
  ...
  for Ang=AnglesVecGrid
    File   = strcat('./QSSRates_To_PES_CorrCoeff.csv.', num2str(Ang));
    fileID = fopen(File,'w');
    fprintf(fileID,'R1,R2,R3,EMean,EMinus,EPlus,KDissCorr,KDissQSSCorr,KExchCorr,KExchQSSCorr\n');
    ...
    for iR1=1:NPointsGrid
      for iR3=iR1:NPointsGrid
        ...
        corr                  = corrcoef(A2,B);
        KDissQSSCorr(iPoints) = corr(1,2);
        corr               = corrcoef(A3,B);
        KExchCorr(iPoints) = corr(1,2);
        corr                  = corrcoef(A4,B);
        KExchQSSCorr(iPoints) = corr(1,2);
        
        fprintf(fileID,'%e,%e,%e,%e,%e,%e,%e,%e,%e,%e\n', R1, R2, R3, EMean, EMinus, EPlus, abs(KDissCorr(iPoints)), abs(KDissQSSCorr(iPoints)), abs(KExchCorr(iPoints)), abs(KExchQSSCorr(iPoints)) );
        iPoints=iPoints+1;
      end
    end
  ...
  for iProc=1:4
    ...
    if (NbSampledPoints(iProc) >0 )
      ...
      if iProc==1
        File2   = strcat('./KDissCorr_To_PES_Corr_RPoints.csv');
      elseif iProc==2
        File2   = strcat('./KDissQSSCorr_To_PES_Corr_RPoints.csv');
      elseif iProc==3
        File2   = strcat('./KExchCorr_To_PES_Corr_RPoints.csv');
      elseif iProc==4
        File2   = strcat('./KExchQSSCorr_To_PES_Corr_RPoints.csv');
      end
      ...
      while iSample <= NbSampledPoints(iProc)
       ...
       fprintf('Proc. %i; Number of Tot Attempts: %i; Number of Points Accepted so far: %i\n', iProc, NAttempts, iSample-1);
      ...
      for iAng=AnglesVecGrid
        if iProc==1
          File1   = strcat('./KDissCorr_To_PES_Corr_Points.csv.', num2str(iAng));
        elseif iProc==2
          File1   = strcat('./KDissQSSCorr_To_PES_Corr_Points.csv.', num2str(iAng));
        elseif iProc==3
          File1   = strcat('./KExchCorr_To_PES_Corr_Points.csv.', num2str(iAng));
        elseif iProc==4
          File1   = strcat('./KExchQSSCorr_To_PES_Corr_Points.csv.', num2str(iAng));
        end
