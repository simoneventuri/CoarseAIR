...
function [iFigure, tauInt, tauIntP, tauVib, tauVibP, tauRot, tauRotP] = PlotEnergies(iT, iFigure, t, eInt, eRot, eVib, MolFracs, P, Steps)    
  ...
  for iMol = 1:1 %NBinnedMol
    ...
    Deltat = t(Steps(iVib)) - t(Steps(iVib-1));
    Deltae = eVib(Steps(iVib)) - eVib(Steps(iVib-1));
    Semie  = eVibLim - eVib(Steps(iVib-1));
    tauVib(iMol)  = Deltat ./ Deltae * Semie + t(Steps(iVib-1));
...
%     tauVib(iMol)   = eVibInfo.RiseTime;
    ...
    %%%%(iMol)   = (t(Steps(iRot)) + t(Steps(iRot-1)))./2.d0;
    Deltat = t(Steps(iRot)) - t(Steps(iRot-1));
    Deltae = eRot(Steps(iRot)) - eRot(Steps(iRot-1));
    Semie  = eRotLim - eRot(Steps(iRot-1));
    tauRot(iMol)  = Deltat ./ Deltae * Semie + t(Steps(iRot-1));
...
%     tauRot(iMol)   = eRotInfo.RiseTime;
    ...
    Deltat = t(Steps(iInt)) - t(Steps(iInt-1));
    Deltae = eInt(Steps(iInt)) - eInt(Steps(iInt-1));
    Semie  = eIntLim - eInt(Steps(iInt-1));
    tauInt(iMol)  = Deltat ./ Deltae * Semie + t(Steps(iInt-1));
