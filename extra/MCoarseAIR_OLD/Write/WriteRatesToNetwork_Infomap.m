function [AInel, eigval] = WriteRatesToNetwork_Infomap(iT, NLevels, LevelEeV, Levelvqn, Leveljqn, rIn, Levelg, RatesMatrix, ProcessesRates, LevelEeVVib0, LevelEeVRot, DeltaEintDiss)
...
%   T0    = 1400;
  ...
  MM      = sparse(AInel);
  eigval  = eigs(MM,NLevels(iBinnedMol));
  [U,S,V] = svd(AInel);
  
  max(max(abs( (AInel - U*S*V') ./ AInel ) ))
  tic
