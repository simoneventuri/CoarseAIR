...
PES_NAMES      = [string('11A1'), string('21A1'), string('11A2'), string('13A1'), string('23A1'), string('13A2'), string('15A1'), string('25A1'), string('15A2')];
...
for PES_NAME = PES_NAMES
  ...
  filename = strcat(WORKSPACE_PATH,'/CoarseAIR/coarseair/dtb/O3/PESs/AbInitioData/', PES_NAME(:), '.dat')
  delimiter = ' ';
  formatSpec = '%f%f%f%[^\n\r]';
  ...
  for i=1:5:length(Col1)
    E_1(Col1(i)) = Col2(i) - ConvConst;
    E_2(Col1(i)) = Col3(i) - ConvConst;
  end
%   figure(iFigure)
...
%   pause
  ...
  R2(:) = sqrt(X3(:,1).^2 + X3(:,2).^2);
  R3(:) = sqrt((X2(:,1) - X3(:,1)).^2 + (X2(:,2) - X3(:,2)).^2);
  R       = [R1', R2', R3'];
  RSorted = sort(R,2);
