...
module n4pipNN_module

  contains

!      subroutine n4pipNN(ct,vpes,vpesa,vpesb,vpesc,vn2) !ct in N4 order and angstrom
!   subroutine n4pipNN(x,y,z,vpes,dedx,dedy,dedz,igrad)
   subroutine n4pipNN(R,vpes,dEdR,igrad)

   use nnparam
   implicit none
   integer i,j,k
   real*8 rb(ndim),xbond(ndim),basis(1:nbasis),tmp1, txinput(nbasis-1),bnew(1:nnew)
...
!-->  one has to call this subroutine one and only one before calling the getpot() subroutine
  ...
  open(4,file='/home/venturi/WORKSPACE/CoarseAIR/coarseair/dtb/Systems/N4/PESs/UMN_PIPNN/Biases.dat',status='old')
  rewind(4)
  read(4,'(a80)')line
  write(*,*)line

    nfile=7
    open(nfile,file='/home/venturi/WORKSPACE/CoarseAIR/coarseair/dtb/Systems/N4/PESs/UMN_PIPNN/Weights.dat')
    read(nfile,'(a80)')line
    write(*,*)line
...
! Modified parameters for D3(BJ)
  ...
  fy = as(0) + as(1)*y + as(2)*y*y + as(3)*y*y*y + as(4)*y*y*y*y    + as(5)*y*y*y*y*y + as(6)*y*y*y*y*y*y
...
! Compute the gradient if needed
  if (igrad.eq.1) then
   ...
   dfdy = as(1) + 2.0d0*as(2)*y + 3.0d0*as(3)*y*y       + 4.0d0*as(4)*y*y*y + 5.0d0*as(5)*y*y*y*y       + 6.0d0*as(6)*y*y*y*y*y
...
! Calculate dispersion correction
  call edisp(94,5,2,dist,iz,mxc,      rs6,rs8,e6,e8,e6dr,e8dr,c6,0)
  ...
  if (igrad .eq. 1) then
  call edisp(94,5,2,dist,iz,mxc,      rs6,rs8,e6,e8,e6dr,e8dr,c6,1)
...
!  the old values
  r2r4(1:94)=(/ 2.00734898d0,  1.56637132d0,  5.01986934d0,  3.85379032d0,  3.64446594d0,  3.10492822d0,  2.71175247d0,  2.59361680d0, &
                2.38825250d0,  2.21522516d0,  6.58585536d0,  5.46295967d0,  5.65216669d0,  4.88284902d0,  4.29727576d0,  4.04108902d0, &
                3.72932356d0,  3.44677275d0,  7.97762753d0,  7.07623947d0,  6.60844053d0,  6.28791364d0,  6.07728703d0,  5.54643096d0, &  
                5.80491167d0,  5.58415602d0,  5.41374528d0,  5.28497229d0,  5.22592821d0,  5.09817141d0,  6.12149689d0,  5.54083734d0, &  
                5.06696878d0,  4.87005108d0,  4.59089647d0,  4.31176304d0,  9.55461698d0,  8.67396077d0,  7.97210197d0,  7.43439917d0, &  
                6.58711862d0,  6.19536215d0,  6.01517290d0,  5.81623410d0,  5.65710424d0,  5.52640661d0,  5.44263305d0,  5.58285373d0, &  
                7.02081898d0,  6.46815523d0,  5.98089120d0,  5.81686657d0,  5.53321815d0,  5.25477007d0, 11.02204549d0,  0.15679528d0, &  
                9.35167836d0,  9.06926079d0,  8.97241155d0,  8.90092807d0,  8.85984840d0,  8.81736827d0,  8.79317710d0,  7.89969626d0, &  
                8.80588454d0,  8.42439218d0,  8.54289262d0,  8.47583370d0,  8.45090888d0,  8.47339339d0,  7.83525634d0,  8.20702843d0, &  
                7.70559063d0,  7.32755997d0,  7.03887381d0,  6.68978720d0,  6.05450052d0,  5.88752022d0,  5.70661499d0,  5.78450695d0, &  
                7.79780729d0,  7.26443867d0,  6.78151984d0,  6.67883169d0,  6.39024318d0,  6.09527958d0, 11.79156076d0, 11.10997644d0, &  
                9.51377795d0,  8.67197068d0,  8.77140725d0,  8.65402716d0,  8.53923501d0,  8.85024712d0 /)

! these new data are scaled with k2=4./3.  and converted a_0 via
! autoang=0.52917726d0
  rcov(1:94)=(/ 0.80628308d0, 1.15903197d0, 3.02356173d0, 2.36845659d0,  1.94011865d0, 1.88972601d0, 1.78894056d0, 1.58736983d0, &  
                1.61256616d0, 1.68815527d0, 3.52748848d0, 3.14954334d0,  2.84718717d0, 2.62041997d0, 2.77159820d0, 2.57002732d0, &  
                2.49443835d0, 2.41884923d0, 4.43455700d0, 3.88023730d0,  3.35111422d0, 3.07395437d0, 3.04875805d0, 2.77159820d0, &  
                2.69600923d0, 2.62041997d0, 2.51963467d0, 2.49443835d0,  2.54483100d0, 2.74640188d0, 2.82199085d0, 2.74640188d0, &  
                2.89757982d0, 2.77159820d0, 2.87238349d0, 2.94797246d0,  4.76210950d0, 4.20778980d0, 3.70386304d0, 3.50229216d0, &  
                3.32591790d0, 3.12434702d0, 2.89757982d0, 2.84718717d0,  2.84718717d0, 2.72120556d0, 2.89757982d0, 3.09915070d0, &  
                3.22513231d0, 3.17473967d0, 3.17473967d0, 3.09915070d0,  3.32591790d0, 3.30072128d0, 5.26603625d0, 4.43455700d0, &  
                4.08180818d0, 3.70386304d0, 3.98102289d0, 3.95582657d0,  3.93062995d0, 3.90543362d0, 3.80464833d0, 3.82984466d0, &  
                3.80464833d0, 3.77945201d0, 3.75425569d0, 3.75425569d0,  3.72905937d0, 3.85504098d0, 3.67866672d0, 3.45189952d0, &  
                3.30072128d0, 3.09915070d0, 2.97316878d0, 2.92277614d0,  2.79679452d0, 2.82199085d0, 2.84718717d0, 3.32591790d0, &  
                3.27552496d0, 3.27552496d0, 3.42670319d0, 3.30072128d0,  3.47709584d0, 3.57788113d0, 5.06446567d0, 4.56053862d0, &  
                4.20778980d0, 3.98102289d0, 3.82984466d0, 3.85504098d0,  3.88023730d0, 3.90543362d0 /)
...
program ComputeDiat
  ...
  use n4pipNN_module ,only: n4pipNN, PREPOT
  ...
  double precision,parameter :: Econv2 = 27.2113839712790
  integer                    :: i, j, iA
...
!  
  ...
  do iA=1,size(alphaVec,1)
    ...
    open( File=trim(adjustl(FileName)), NewUnit=Unit, status='REPLACE', iostat=Status )
      write(Unit,'(A)') 'R1,R2,R3,R4,R5,R6,V,dV1,dV2,dV3,dV4,dV6,dV6'
      ...
      do i=1,NPoints1
        ...
        do j=1,NPoints2
          ...
          call n4pipNN(R,E,dVdR,1)
          E    = E    * Econv2
          dVdR = dVdR * Econv2
          write(Unit,'(e20.10,12(A,e20.10))') R(1), ',',    R(2), ',',    R(3), ',',    R(4), ',',    R(5), ',',   R(6), ',', &
                                                                                                                      E, ',', &
                                           dVdR(1), ',', dVdR(2), ',', dVdR(3), ',', dVdR(4), ',', dVdR(5), ',', dVdR(6)
